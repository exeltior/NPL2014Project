...
def compressSentence(sentence, byte_limit):
    ...
    while (len(sentence) > byte_limit and can_prune == True):        
        ...
        for sub in t.subtrees(filter=lambda x: len(x.leaves()) and x.leaves()[0].lower() != 'to' and
                              ...
                              or (x.node == 'JJ')
                              or (x.node == 'SBAR' and not listIntersect(x.parent().leaves(), announcetags) and (x.left_sibling() == None or (x.left_sibling().node != 'VBG')))
                              or (x.node == 'S' and (x.left_sibling() == None or (x.left_sibling().node != 'VBG' and x.left_sibling().node != 'IN')))
